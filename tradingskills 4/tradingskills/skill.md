...
#### 5.6.1 ML-for-alpha — generating the primary signal (beyond meta-labeling; `agent.md §8.3` gives the operational summary)
Meta-labeling only filters an *existing* edge. Using ML to *produce* the primary return forecast adds whole failure classes on top of the standard backtest traps (§5.1). None of the quantities below are quotable without running the pipeline (`agent.md §8.3` HARD RULE) — `[needs computation]` everywhere.

- **Stationarity vs. memory — fractional differentiation.** Raw price levels are non-stationary (a tree/boosting model extrapolates garbage); naive first-differencing makes a series stationary but **erases the memory/trend** the model needs. **Fractional differentiation** finds the lowest order `d ∈ (0,1]` that achieves stationarity (ADF/KPSS) while retaining maximum memory — a fractional-differenced feature keeps far more predictive signal than `Δprice`. Fit `d` per series; it is a real estimation step, not a hyperparameter to eyeball. `[needs computation]`
- **Leakage classes that purging/embargoing does NOT catch.** The overlap-label purge (above) handles label-window bleed. It does **not** handle: **group/cross-sectional leakage** (a feature computed over a panel uses same-tick information across assets that re-enters the train fold); **feature-engineering look-ahead** (normalizing/standardizing a feature on a window that includes the test period — fit scalers/encoders on **train only**, apply to test); **target-derived features** (anything constructed using the realized outcome); and **universe survivorship** (training on tokens that still trade, dropping the delisted majority — §5.1 survivorship). Each inflates CV accuracy silently and is invisible to a naive K-fold.
- **Gradient-boosted return models — the workhorse, and why it decays fast in crypto.** Tree ensembles (XGBoost/LightGBM/CatBoost) are the practical default for cross-sectional return prediction. Crypto-specific failure modes: **(1) violent non-stationarity** — the fitted relationship has a short half-life, so the deliverable is a *continuously retrained training pipeline*, not a static model; constant walk-forward retraining is mandatory, not optional; **(2) feature decay** — yesterday's strongest predictor (a specific funding proxy, a narrative feature) is frequently today's noise, so monitor feature-importance drift as a kill/deploy signal; **(3) smooth-then-cliff payoff** — a model that scores well until a regime absent from its training window arrives, so always evaluate on a held-out *regime*, not just a held-out *time*. **(4) cross-validated performance ≠ live performance** — even leak-free CV overstates live returns because it omits execution cost, impact at size, and the crowding/decay that begins the moment a real edge is traded.
- **The honest baseline.** Before trusting any ML alpha, verify it clears: fractional-diffed or otherwise-stationary features, train-only feature engineering, purged+embargoed+combinatorial CV, a **purged out-of-sample holdout spanning >1 regime**, and a positive E[R] **net of the §5.2 cost model** with a deflated-Sharpe / PBO check for the search. `[needs computation]` for every one. An ML "alpha" that hasn't survived these is a curve, not an edge.
...
- **Order-type selection**: maker (limit/post-only) to earn spread/rebate when not urgent; taker (market) only when immediacy > cost. Stop-*limit* can leave you unprotected in a gap; a stop-*market* fills more reliably but is still only **SOFT / best-effort risk control, NOT a guarantee.** In fast/cascade conditions stops **gap and slip 2–5× past their trigger**, and exit can be outright **impossible mid-cascade** (no bid, halted matching, liquidation queue). Treat every stop as best-effort and **size assuming it fills materially worse than its level**, per the **stop-slippage buffer** (`agent.md §6.0`: floor `1.5–3×` for back-of-envelope, binding = tail-adaptive formula when data is available — see §7).
...
> **All risk limits here are governed by the CANONICAL RISK REGISTER — master copy `agent.md §6.0`; circuit breakers `agent.md §6.4`.** Use those EXACT numbers; this section **references, never re-sources**. The single local definition needed to read the formulas below: **1R ≡ the % of account equity lost if the stop fills at its level** (express every risk in both R and its % equivalent). Risk-per-trade, the 2% ceiling, heat caps, Kelly fraction, the stop-slippage buffer, and the per-venue counterparty cap all live in `agent.md §6.0` — go there for the values; **when any limits conflict, the TIGHTEST binds.** These are default *priors*, not laws or edge; recalibrate to the strategy's own measured drawdown distribution.
>
> **Single source of truth (anti-drift — governs how to edit this file).** The numbers in this section are a **read-only mirror** of the register in `agent.md`. **If any figure here ever disagrees with `agent.md`, `agent.md` wins and the tighter number binds** — fix the mirror, never fork it. Do not edit a constant here without editing the master.
...
slip_mult        = 1.5–3                    # MINIMUM FLOOR only — the stop-slippage buffer
                                            #   (agent.md §6.0). Use the tail-adaptive formula
                                            #   (MAX(3x, 99th-pct wick/avg wick)·(size/L1 depth))
                                            #   as the BINDING value when depth/wick data exists.
...
> **Leverage is NOT a sizing tool.** Position size is set ONLY by risk-per-trade ÷ stop-distance; leverage merely sets margin & liquidation distance. **BINDING RULE (`agent.md §6.0/§6.2`):** the liquidation price must sit beyond the stop/invalidation by the **stop-slippage buffer** — **floor `1.5–3×` for back-of-envelope, binding = the tail-adaptive formula when depth/wick data is available** — such that the buffer still does NOT liquidate; **REJECT** any size/leverage that puts liquidation inside (stop + buffer). Effective-leverage caps per the register (`agent.md §6.0`).
...
Per-trade sizing and heat caps are **not enough** — they bound one trade, not a bad day or a feed failure. The bundle's account-level halts (daily-loss limit, consecutive-loss halt, drawdown ladder) are **binding survival constraints** whose **master copy and exact bands live in `agent.md §6.4`** — go there for the figures; this section **references, never re-lists** them.

What matters here is the *character* of these breakers, not their numbers: they are **mechanical trips, not judgment calls** — a breaker fires regardless of how good the next setup looks, the **tightest applicable breaker binds**, and it resets only at the proper boundary (session / completed review), never on "I feel better now." Tie ruin (§7) and tilt to these *actions*. (Operating-loop wiring is owned by `agent.md` §6.4; if it were ever absent there, that would be a bundle-level blow-up hole — it is present and binding.)
